...
import { advanceDateRangeSelection } from './dateRangeSelection';

/**
 * Calendar body for a custom date window. Dates are local `YYYY-MM-DD`.
 * Hosted inside `TimeRangePicker` (do not mount a second modal).
 *
 * @param {{
 *   active?: boolean;
...
export function CustomDateRangeForm({
  ...
  const canView = isCompleteCustomDateRange(rangeStartKey, rangeEndKey);
  const selectionSummary = rangeStartKey
    ? formatCustomDateRangeLabel(rangeStartKey, rangeEndKey || rangeStartKey)
    : '';
  ...
  const handleSelectDateKey = useCallback(
    (key) => {
      const next = advanceDateRangeSelection(rangeStartKey, rangeEndKey, key);
  ...
  useEffect(() => {
    ...
    onConfirmRef.current = () => {
      if (!isCompleteCustomDateRange(rangeStartKey, rangeEndKey)) return null;
