...
| Foreground banner behavior                        | `index.js` — `Notifications.setNotificationHandler` (banner + **sound on**; do not silence). OS plays sound/vibration from the server payload (`sound: "default"`).                                                                                                                                                 |
| Android alert channel                             | `ensureAndroidDefaultNotificationChannel` — channel id **`default`** (name **ServiceLink**), sound + vibration + high importance. Recreates the channel if it was created silent. Server must send `channelId: "default"`.                                                                                          |
