...
/**
 * Start or resume Stripe Connect Express onboarding (mobile).
 *
 * POST `/api/stripe/connect/onboard` with `{ "client": "mobile" }`.
 *
 * Contract v2 note: for `client: "mobile"`, the web server should mint Account Links with HTTPS
 * bridge routes (`/mobile-bridge/connect-return` + `/mobile-bridge/connect-refresh`). Bridge pages
 * then deep-link back to the app. Stripe rejects custom schemes directly in Account Link
 * `return_url` / `refresh_url`.
 *
 * @param {string | null | undefined} accessToken
 * @returns {Promise<{ url: string } | { error: Error; httpStatus: number }>}
 */
export async function postStripeConnectOnboard(accessToken) {
  ...
  let res;
  try {
    res = await fetch(`${origin}/api/stripe/connect/onboard`, {
      method: 'POST',
      headers: {
