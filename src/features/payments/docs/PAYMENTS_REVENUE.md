...
| **Ranges**       | Week (Mon–Sun), Month, Year, All time. Default: **Month**.                                                                                                |
| **Compare**      | Week / Month / Year show % vs prior period. All time has no compare.                                                                                      |
...
1. **New / empty business** — Revenue shows `$0`, flat chart, Jobs paid `0`, empty caption. Switching Week/Month/Year/All stays empty.
...
3. **Free user** — Can open Revenue; Settings shows web upsell (no in-app subscribe CTA).
4. **Pro, no Connect** — Revenue works; Settings shows Connect setup.
5. **Pro + Connect** — Revenue + full Settings.
...
| `components/PaymentsRevenueRangePicker.jsx` | Week / Month / Year / All time sheet                   |
...
| `queryKeys.js`                              | `paymentsRevenueQueryKey(businessId, range)`           |
...
Previous-period fetch runs for Week / Month / Year so the UI can show % change. All time skips it.
...
- Query key: `['payments', 'revenue', businessId, range]`
- `staleTime`: 60s · `gcTime`: 15m
- Enabled only when `businessId` is set

## Tests

| File                                              | Covers                                                                   |
| ------------------------------------------------- | ------------------------------------------------------------------------ |
| `__tests__/aggregatePaymentsRevenue.test.js`      | Empty, week/month/year/all, ignore non-completed, change %, same-day sum |
| `__tests__/revenueDateWindows.test.js`            | Window bounds + Monday week start                                        |
| `__tests__/fetchCompletedBookingPayments.test.js` | Query filters, missing businessId, errors                                |
| `__tests__/PaymentsRevenueSection.test.jsx`       | Empty caption vs populated UI                                            |
