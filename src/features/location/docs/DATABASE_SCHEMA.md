...
Add a new column to track if the user has dismissed the location prompt:

```sql
ALTER TABLE profiles
ADD COLUMN IF NOT EXISTS location_prompt_dismissed BOOLEAN DEFAULT FALSE;

COMMENT ON COLUMN profiles.location_prompt_dismissed IS 'Whether the user has dismissed the location collection prompt';
```

**Notes:**
- This prevents showing the location prompt repeatedly after the user dismisses it
- Defaults to FALSE so the prompt shows for existing users who haven't provided location yet
- Can be reset to FALSE if we want to re-prompt users in the future
...
### Location Save
1. User enters city, state, and radius in modal
...
2. App updates `profiles.location_prompt_dismissed` = TRUE
3. Modal won't show again for this user
...
- [ ] Add `location_prompt_dismissed` column to `profiles`
