...
function assertOriginIsAllowed(origin) {
  ...
  if (!__DEV__) {
    if (parsed.protocol !== 'https:') {
      throw new Error('Production billing portal requires an https web origin');
    }
    if (LOCAL_HOSTS.has(parsed.hostname)) {
      throw new Error('Production billing portal cannot target localhost');
    }
  }
}

/**
 * POST `/api/stripe/create-portal-session` — hosted Stripe Billing Portal session URL.
 *
 ...
 */
export async function createBillingPortalSession(accessToken) {
  ...
  let res;
  try {
    res = await fetch(`${origin}/api/stripe/create-portal-session`, {
      method: 'POST',
      headers: {
